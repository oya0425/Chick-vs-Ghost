...
#include"../Character/RigidbodyComponent.h"
...
namespace {
	...
	constexpr float fontOffset = -8.0f;
	...
	//ゲーム本番ボタン
	constexpr float gamePlay_button_x = 850.0f;
	constexpr float gamePlay_button_y = 600.0f;
	...
	constexpr float button_h = 70.0f;

	//======================================================================
	//総撃破数の背景
	constexpr float enemyKillCountBack_x = 430.0f;		
	constexpr float enemyKillCountBack_y = 110.0f;
	constexpr float enemyKillCountBack_max_w = 240.0f;
	constexpr float enemyKillCountBack_min_w = 240.0f;
	...
	constexpr float timeBackGround_w = 150.0f;
	constexpr float timeBackGround_h = 150.0f;
	constexpr float timeBackGround_y = 120.0f;
	...
	// --- ボスHPバー ---
	constexpr float barWidthBossHp = 400.0f;
	constexpr float barHeightBossHp = 36.0f;

	constexpr float barBackWidthBossHp = 408.0f;
	constexpr float barBackHeightBossHp = 42.0f;

	// 右端
	constexpr float barRightEdgeBoss = 1200.0f;

	constexpr float bossHeigtYHp = 60.0f;


	// ボスバーの中心位置
	constexpr float newBarPosXBoss =
		barRightEdgeBoss - (barWidthBossHp * 0.5f);

	constexpr float newBarBackPosXBoss =
		barRightEdgeBoss - (barBackWidthBossHp * 0.5f);
	
	//HPバーを分割するため
	constexpr float sectionWidth = barWidthBossHp / 5.0f;
	constexpr float divideWidth = 5.0f;
	...
	// --- スキルのクールタイムバー ---
	// --- 範囲攻撃スキル（Area Attack）用バー ---
	//縦幅
	constexpr float skillBarThick = 30.0f; 
	// バーの左端のX座標
	// X座標
	constexpr float barLeftEdgeAreaSkill = 100.0f;
	// Y座標
	constexpr float heightYAreaSkill = 320.0f + operationUI_Y;

	// クールタイムが最大のときのバーの最大横幅
	constexpr float maxWAreaSkill = 200.0f;


	// --- 引き寄せスキル（Pull）用バー ---
	// X座標
	constexpr float barLeftEdgePullSkill = 100.0f;

	// Y座標
	constexpr float heightYPullSkill = 400.0f + operationUI_Y;

	// クールタイムが最大のときのバーの最大横幅
	constexpr float maxWPullSkill = 200.0f;
	...
	//最終WAVEの特攻状態で出てくる敵の数を増やす倍率（ボスの近接耐性に掛ける）
	constexpr float finalChargeEnemyNum = 2.0f;	
...
void SceneMain::SetupEnemy(NewEnemyClass* enemy, const NewEnemyClass::EnemyData& data,bool isLeader,bool isBoss)
{
	...
	if (isLeader && isBoss)
	{
		...
		NewEnemyClass::GroupColorData colorData = enemy->GetRandomGroupDataColor();
...
void SceneMain::InitializeField()
{
	...
	for (int line = 0; line < 2; line++)
	{
		for (int i = 0; i < TREE_NUM; i++)
		{
			...
			pTree[index] = new vnModel(L"data/model/Tree/", L"ST_Tree_01_LOD0.vnm");

			float radian = (2.0f * XM_PI / (float)TREE_NUM) * (float)i;
			if (line == 1)
			{
				float halfGap = (2.0f * XM_PI / (float)TREE_NUM / 2.0f);
				radian += halfGap;
			}
...
{
	InitializeBarUI();			// HP・EXP・ボスHP
	InitializeGameUI();         // アイコン・コンボ
	InitializeKeyBoardUI();     // 背景・キーボード
	InitializeSkillUI();        // スキル
	InitializeInfoUI();         // 時間・撃破数
}

//HPバー（Exp）設定
void SceneMain::InitializeBarUI()
{
	...
	//ボス
	pBossHpBarBackBlack = new vnSprite(newBarBackPosXBoss, bossHeigtYHp, barBackWidthBossHp, barBackHeightBossHp, NULL);
	pBossHpBarBackBlack->setColor(V_GAME_COLOR_BLACK);
	//pBossHpBarBackBlack->setSkewX(12.0f);
	registerObject(pBossHpBarBackBlack);


	pBossHpBarBack = new vnSprite(newBarPosXBoss, bossHeigtYHp, barWidthBossHp, barHeightBossHp, NULL);
	pBossHpBarBack->setColor(V_GAME_COLOR_RED);
	//pBossHpBarBack->setSkewX(9.8f);
	registerObject(pBossHpBarBack);


	pBossHpBarFront = new vnSprite(newBarPosXBoss, bossHeigtYHp, barWidthBossHp, barHeightBossHp, NULL);
	pBossHpBarFront->setColor(V_GAME_COLOR_GREEN);
	//pBossHpBarFront->setSkewX(9.8f);
	registerObject(pBossHpBarFront);
	...
	for (int i = 1; i < 5; i++)
	{
		...
		divide[i - 1]->setColor(V_GAME_COLOR_BLACK);

		registerObject(divide[i - 1]);
	}
...
//アイコン・コンボ
void SceneMain::InitializeGameUI()
{
	//================================================
	// --- プレイヤーアイコン ---
	//================================================
	...
	// --- ボスアイコン ---
	//================================================
	pIconBoss = new vnSprite(1240, 40, 70, 70, L"data/image/ghostImage.png");
	registerObject(pIconBoss);

	SetBossHPbarRender(false);
...
//操作の画像の背景・キーボード
void SceneMain::InitializeKeyBoardUI()
{
	//===============================================================
	// --- 文字用背景(左側にある操作説明の後ろにある背景)---
	//===============================================================
	m_pUIBackGroundBlack[0] = new vnSprite(1280.0f - 1120.0f, 720.0f - 450.0f, 256.0f * 1.2f, 512.0f * 0.7f, L"BackGroundBlack.png");
	...
	//右下側にあるメッセージの表示の背景
	m_pUIBackGroundBlack[1] = new vnSprite(1280.0f - 170.0f, 720.0f - 110.0f, 256.0f * 1.2f, 512.0f * 0.3f, L"BackGroundBlack.png");
	...
	// --- キーボード画像 ---
	//============================================================
	pImageW =	new vnSprite(-100.0f, 200.0f, 64.0f, 64.0f, L"data/image/keyboard_w_outline.png"); registerObject(pImageW);
	pImageA =	new vnSprite(-100.0f, 200.0f, 64.0f, 64.0f, L"data/image/keyboard_a_outline.png"); registerObject(pImageA);
	pImageS =	new vnSprite(-100.0f, 200.0f, 64.0f, 64.0f, L"data/image/keyboard_s_outline.png"); registerObject(pImageS);
	pImageD =	new vnSprite(-100.0f, 200.0f, 64.0f, 64.0f, L"data/image/keyboard_d_outline.png"); registerObject(pImageD);
	pImageE =	new vnSprite(-100.0f, 200.0f, 64.0f * 0.7f, 64.0f * 0.7f, L"data/image/keyboard_e_outline.png"); registerObject(pImageE);
	pImageQ =	new vnSprite(-100.0f, 200.0f, 64.0f * 0.7f, 64.0f * 0.7f, L"data/image/keyboard_q_outline.png"); registerObject(pImageQ);
	pImageTab = new vnSprite(-100.0f, 200.0f, 64.0f * 1.5f, 64.0f * 1.5f, L"data/image/keyboard_tab.png"); registerObject(pImageTab);

}

//プレイヤーのスキル
void SceneMain::InitializeSkillUI()
{
	//===================================================================
	// --- スキルUI設定 ---
	//===================================================================
	...
	pAreaSkillIcon = new vnSprite(centerX - 150.0f, heightYAreaSkill - 15, 64 * 1.2f, 64 * 1.2f, L"data/image/areaAttackIconImg2.png");
	...
	pPullSkillIcon = new vnSprite(centerX - 150.0f, heightYPullSkill - 15, 64 * 1.2f, 64 * 1.2f, L"data/image/pullAttackIconImg2.png");
...
//時間表示の背景・撃破数の後ろの背景
void SceneMain::InitializeInfoUI()
{
	...
	//総撃破数表示の後ろの背景
	//===============================================================
	pEnemyKillCountBack = new vnSprite(enemyKillCountBack_x, enemyKillCountBack_y, enemyKillCountBack_max_w, 40, L"data/image/mission_frame.png");
...
//ミッションのUIの初期化(ミッションクリアの基準など)
void SceneMain::InitializeMissionUI()
{
	AddMissionImage();
	// ミッション内容のテキストを入れる
	m_missionUI[(int)TutorialMission::WaitEnemyLeaderKill].text = L"リーダーを１体倒そう";
	...
	m_missionUI[(int)TutorialMission::WaitDamage].text = L"ダメージを受けよう";
	m_missionUI[(int)TutorialMission::WaitEnemyCount].text = L"敵を合計200体倒そう";

	// ミッションの状況を設定
	m_missionUI[(int)TutorialMission::WaitEnemyLeaderKill].current = 0;
	m_missionUI[(int)TutorialMission::WaitEnemyLeaderKill].target = 1;

	m_missionUI[(int)TutorialMission::WaitSkill_Area].current = 0;
	m_missionUI[(int)TutorialMission::WaitSkill_Area].target = 1;

	m_missionUI[(int)TutorialMission::WaitSkill_Pull].current = 0;
	m_missionUI[(int)TutorialMission::WaitSkill_Pull].target = 1;

	m_missionUI[(int)TutorialMission::WaitLevelUp].current = 0;
	m_missionUI[(int)TutorialMission::WaitLevelUp].target = 1;

	m_missionUI[(int)TutorialMission::WaitEnemyRange].current = 0;
	m_missionUI[(int)TutorialMission::WaitEnemyRange].target = 1;

	m_missionUI[(int)TutorialMission::WaitDamage].current = 0;
	m_missionUI[(int)TutorialMission::WaitDamage].target = 1;

	m_missionUI[(int)TutorialMission::WaitEnemyCount].current = 0;
	m_missionUI[(int)TutorialMission::WaitEnemyCount].target = 200;
}
...
void SceneMain::InitializePauseUI()
{
	...
	enemyPool->SetImageTab(new vnSprite(-100.0f, 200.0f, 64.0f * 1.5f, 64.0f * 1.5f, L"data/image/keyboard_tab.png"));
	//enemyPool->SetImageTab(pImageTab);
	enemyPool->SetImageA(new vnSprite(-100.0f, 200.0f, 64.0f * offsetSize, 64.0f * offsetSize, L"data/image/keyboard_a.png"));
	enemyPool->SetImageD(new vnSprite(-100.0f, 200.0f, 64.0f * offsetSize, 64.0f * offsetSize, L"data/image/keyboard_d.png"));
	enemyPool->SetImageSlash(new vnSprite(-100.0f, 200.0f, 64.0f * offsetSize, 64.0f * offsetSize, L"data/image/flair_disabled_line_outline.png"));
	enemyPool->SetImageGhost(new vnSprite(-100.0f, 200.0f, 64.0f * offsetSizeGhost, 64.0f * offsetSizeGhost, L"data/image/ghostImage.png"));
...
void SceneMain::InitializeExplanationUI()
{
	...
	AddExplanationImage(ExplanationType::Skills, L"data/image/tutorial_ExplainSkills2.png");
	...
	AddExplanationImage(ExplanationType::Boss, L"data/image/tutorial_ExplainBoss.png");
	...
	//チュートリアル振り返りボタン
	{
		...
		InitButton(UIButton::TUTORIAL_REVIEW,
			tutorial_review_button_x, tutorial_review_button_y, L"遊び方",threeLetters);
		...
		InitButton(UIButton::TUTORIAL_BOSS,
			tutorial_boss_button_x, tutorial_boss_button_y, L"ボス",-10.0f);
	}


	//==========================================================================
	// --- チュートリアルクリア時に出てくる文字などのアニメーション処理用 ---
	//==========================================================================
	m_tutorialClearUI.position_x = screenWidth / 2-600;
	m_tutorialClearUI.position_y = screenHeight / 2;
...
void SceneMain::AddMissionImage()
{
	...
	for (int i = 0; i < (int)TutorialMission::MaxNum; i++)
	{
		...
		m_missionUI[i].sprite_back->setAlpha(0.7f);
		registerObject(m_missionUI[i].sprite_back);

		m_missionUI[i].position_x = posX;
		m_missionUI[i].position_y = 60 + (i * sizeY);

		// チェックボックス画像
		m_missionUI[i].sprite_check_box = new vnSprite(
			m_missionUI[i].position_x-175,
			m_missionUI[i].position_y,
			sizeY-10,
			sizeY-10,
			L"data/image/checkBox.png");

		m_missionUI[i].sprite_check_box->setRenderEnable(false);
		m_missionUI[i].sprite_check_box->setAlpha(0.7f);
		registerObject(m_missionUI[i].sprite_check_box);
		...
		m_missionUI[i].sprite_check = new vnSprite(
			...
			sizeY,
			sizeY,
			L"data/image/checkMark.png");

		m_missionUI[i].sprite_check->setRenderEnable(false);
		m_missionUI[i].sprite_check->setAlpha(1.0f);
		m_missionUI[i].sprite_check->setColor(V_GAME_COLOR_RED);
...
void SceneMain::InitializeTutorial()
{
	...
	//チュートリアルから始めたら無敵状態にする
	m_pNewPlayer->SetIsMuteki(m_isTutorial);
...
void SceneMain::terminate()
{
	...
	for (int i = 0; i < (int)TutorialMission::MaxNum; i++)
	{
		...
		if (m_missionUI[i].sprite_check_box!= nullptr)
		{
			deleteObject(m_missionUI[i].sprite_check_box);
			m_missionUI[i].sprite_check_box =nullptr;
		}
...
void SceneMain::execute()
{
	...
	case LevelUp:
		...
		// プレイヤーの向いている方向から180度反対側にする
		// モデルの向きによる90度のズレを補正する
	...
	const bool isTimeStop = (m_gameState == GameState::TimeStop);
...
void SceneMain::render()
{
	...
	float baseX_WASD = 120.0f;	//WASDの画像の位置（X）
	...
	case Play:
	{
		...
		// --- スキルバーの表示更新 ---
		{
			...
			UpdateSkillBar(
			    ...
			    m_pNewPlayer->GetIsHaveAreaAtkSkill(),
				pAreaSkillIcon,
				pAreaAtkBtnFront,
			...
			UpdateSkillBar(
				...
				m_pNewPlayer->GetIsHavePullSkill(),
				pPullSkillIcon,
				pPullBtnFront,
		...
		if (waveManager->GetFinalWave())
		{
			...
			if (remainBoss > 0)
			{
				...
				// 必要なボス撃破数
				int bossMaxCount =
					waveManager->GetKillNeedBossCount();

				// ゲージ割合
				float hpRatio =
					(float)remainBoss / bossMaxCount;

				// ゲージ本体を縮める
				pBossHpBarFront->setScaleX(hpRatio);
				...
				pBossHpBarFront->setPos(
					frontPosX + shakeX,
					bossHeigtYHp + shakeY);

				pBossHpBarBackBlack->setPos(
					backPosX + shakeX,
					bossHeigtYHp + shakeY);

				pBossHpBarBack->setPos(
					backPosX + shakeX,
					bossHeigtYHp + shakeY);
			}
			else
		...
		if (m_isTutorial)
		{
			...
			for (int i = 0; i < (int)TutorialMission::MaxNum; i++)
			{
				float missionTextX = m_missionUI[i].position_x - 150.0f;
				float missionProgressX = m_missionUI[i].position_x + 110.0f;
				...
				vnFont::print(
					...
					mission.position_y - 10.0f + off + fontOffset,
					GAME_COLOR_WHITE,
				...
				//影
				vnFont::print(
					missionProgressX + mission.font_pos_offset_x + off,
					mission.position_y - 10.0f + off + fontOffset,
					GAME_COLOR_WHITE,
					mission.GetMissionProgress().c_str());
				//本体
				vnFont::print(
					missionProgressX + mission.font_pos_offset_x,
					mission.position_y - 10.0f + fontOffset,
					GAME_COLOR_BLACK,
					mission.GetMissionProgress().c_str());
...
void SceneMain::PlayerSkillSetColorAlpha()
{
	...
	if (m_pNewPlayer->CoolDownAreaAttack())
	{
		pAreaAtkBtnBackBlack->setAlpha(0.5f);
		pAreaAtkBtnBack->setAlpha(0.5f);
		pAreaAtkBtnFront->setAlpha(0.5f);
		pAreaSkillIcon->setAlpha(0.5f);
	}
	...
	if (m_pNewPlayer->CoolDownPull())
	{
		pPullBtnBackBlack->setAlpha(0.5f);
		pPullBtnBack->setAlpha(0.5f);
		pPullBtnFront->setAlpha(0.5f);
		pPullSkillIcon->setAlpha(0.5f);
	}
...
)
{
	...
	if (skillRatio >= 1.0f)
	{
		//if (!isSkillMaxPrev)
		{
			...
			//アイコンの拡大縮小を繰り返す
			float maxTargetScale = 1.20f;
			skillIconScale +=
				(skillTargetScale - skillIconScale) * 0.2f;
			if (fabs(skillIconScale - skillTargetScale) < 0.01f)
			{
				if (skillTargetScale == maxTargetScale)
				{
					skillTargetScale = 1.0f;
				}
				else
				{
					skillTargetScale = maxTargetScale;
				}
			}
		}
...
//==================================================================================
// --- ウィンドウの拡大縮小(開いたら、閉じるまでがセット（2回呼び出しで完了）)---
//==================================================================================
		...
		case WindowMode::Open:
		{
			...
			if (t >= 1.0f)
			{
				...
				if (text != nullptr && text[0] != L'\0')
				{
					...
					vnFont::print(
						screenWidth / 2.0f - 155.0f,
						screenHeight / 2.0f - 40.0f+fontOffset,
						GAME_COLOR_WHITE,
						text);
				}
...
void SceneMain::UpdateExplanationSlide(ExplanationType type)
{
	...
	// 前の画像
	vnSprite* prevImage = nullptr;

	if (m_explanationSlidePage > 0 &&
		m_explanationSlidePage - 1 < explanation.images.size())
	{
		prevImage = explanation.images[m_explanationSlidePage - 1];
	}
...
//ミッションクリアしたときのアニメーション処理
void SceneMain::UpdateMissionAnimation(float deltaTime)
{
	//少し待つ
	// →チェックを拡大縮小する
	// 　→UI全体を左に移動する
	...
	{
		switch (mission.animState)
		{
		case MissionAnimState::CheckWait:
		{
			mission.animTimer += deltaTime;
		...
		case MissionAnimState::Check:

		{
			// チェックの画像を拡大して戻す
			mission.checkScale +=
				(1.5f - mission.checkScale) * 0.2f;

			mission.sprite_check->setScale(mission.checkScale);

			if (mission.checkScale >= 1.49f)
			{
				mission.checkScale = 1.2f;
				mission.animTimer = 0.0f;
				mission.sprite_check->setScale(mission.checkScale);


				mission.animState = MissionAnimState::MoveLeft;
			}

			break;
		}

		case MissionAnimState::ClearText:
			// CLEAR表示

			break;

		case MissionAnimState::MoveLeft:
		{
			//ミッションの１項目のUI全体を左に少し移動する
			mission.animTimer += deltaTime;
			float targetX = mission.oldPosition_x - 500.0f;

			if (MoveMissionUI(mission, targetX, 0.2f))
			{
				mission.position_x = targetX;
				mission.animState = MissionAnimState::MoveRight;
			}


			break;
		}
		case MissionAnimState::MoveRight:
			// 右へ
		{
			// ミッションのUI全体を右へ移動
			mission.animTimer += deltaTime;

			float targetX = vnMainFrame::screenWidth + 300.0f;

			if (MoveMissionUI(mission, targetX, 0.15f))
			{
				mission.position_x = targetX;
				mission.visible = true;
				mission.animState = MissionAnimState::Finished;
			}
	...
	// 残っているミッションを上に詰める
	// 基準の高さ（ミッションの初期状態で一番上にあるやつ）
	float targetY = m_missionUI[0].position_y;
...
//ミッションクリア時に左右移動する
bool SceneMain::MoveMissionUI(
	MissionUI& mission,
	float targetX,
	float speed)
{
	mission.position_x +=
		(targetX - mission.position_x) * speed;
	...
	return fabs(mission.position_x - targetX) < 1.0f;
}
...
void SceneMain::UpdateMission(float deltaTime)
{
	...
	m_missionUI[(int)TutorialMission::WaitSkill_Pull].current = m_pNewPlayer->GetPullUseCount();
	m_missionUI[(int)TutorialMission::WaitLevelUp].current = m_pExpManager->GetLevelUpCount();
	m_missionUI[(int)TutorialMission::WaitEnemyRange].current = enemyPool->GetChargeCount();
	m_missionUI[(int)TutorialMission::WaitDamage].current =	m_pNewPlayer->GetCurrentHp() < m_pNewPlayer->GetMaxHp() ? 1 : 0;
	m_missionUI[(int)TutorialMission::WaitEnemyCount].current = waveManager->GetKillCount();

	//クリアしたミッションをアニメーションしてから消えるようにする
	for (int i = 0; i < (int)TutorialMission::MaxNum; i++)
	{
		auto& mission = m_missionUI[i];
		...
		{
			//ミッションUIの初期化（アニメーション処理するための）
			mission.isClear = true;
			mission.animTimer = 0.0f;
			mission.checkScale = 0.0f;
			mission.oldPosition_x = mission.position_x;
			mission.oldPosition_y = mission.position_y;
			mission.sprite_check->setScale(0.0f);
			mission.animState = MissionAnimState::CheckWait;
		}
	}
	
	UpdateMissionAnimation(deltaTime);
	...
	//チュートリアル終了時に「いいえ」を押した後に本番へ行けるようにボタンを表示
	if (!m_tutorialClearUI.isPlay)
	{
		m_buttonData[(int)UIButton::MESSAGE_GAMEPLAY].position_x = 1100.0f;
		m_buttonData[(int)UIButton::MESSAGE_GAMEPLAY].position_y = 50.0f;
		float posX=m_buttonData[(int)UIButton::MESSAGE_GAMEPLAY].position_x;
		float posY=m_buttonData[(int)UIButton::MESSAGE_GAMEPLAY].position_y;
		m_buttonData[(int)UIButton::MESSAGE_GAMEPLAY].sprite->setPos(posX, posY);
...
void SceneMain::TutorialClearAnim(float deltaTime)
{

	switch (m_tutorialClearUI.state)
	{
	case TutorialClearState::None:
		break;

	case TutorialClearState::ScaleUp:
	{
		// 文字を徐々に拡大
		...
		{
			m_tutorialClearUI.textScale = 1.0f;
			m_tutorialClearUI.state = TutorialClearState::MoveUp;
		}
	...
	{
		float targetY =
			m_tutorialClearUI.oldPosition_y - 200.0f;

		// 上へ移動
		m_tutorialClearUI.position_y +=
			(targetY - m_tutorialClearUI.position_y) * 0.2f;

		// 目標位置にほぼ到達
		if (fabsf(m_tutorialClearUI.position_y - targetY) < 1.0f)
		{
			m_tutorialClearUI.position_y = targetY;
	...
	case TutorialClearState::Confirm:
	{
		m_tutorialClearUI.animTime += deltaTime;
		...
		m_tutorialClearUI.position_y =
			(m_tutorialClearUI.oldPosition_y - 200.0f) +
			sinf(m_tutorialClearUI.animTime * 3.0f) * 10.0f;
		...
		if (m_windowMode == WindowMode::Open)
		{
			...
			if (UpdateButton(UIButton::MESSAGE_GAMEPLAY))
			{
				m_tutorialClearUI.isPlay = true;
				m_windowMode = WindowMode::Close;
			}
			if (UpdateButton(UIButton::MESSAGE_NO))
			{
				m_tutorialClearUI.isPlay = false;
				m_windowMode = WindowMode::Close;
			}
...
void SceneMain::StartCameraRote()
{
	...
	Common::UpdateFlexibleCamera(&pos, phi, radius * 1.5f, theta, FenceRadius);
}
	...
	case ReturnTitleState::Confirm:
	{
		...
		else if (m_windowMode == WindowMode::Close)
		{
			...
			if (UpdateMessageWindow(0.4f, L"", WindowMode::Close, &m_explanationUI[(int)ExplanationType::Exp]))
			{
				...
				////いいえボタンを最低限初期化する
				//noButton.isOn = false;
				//noButton.sprite->setColor(V_GAME_COLOR_BLACK);
...
void SceneMain::UpdatePlayer(float deltaTime)
{
	...
	{
		pDustEmitter->Desc.SizeMax = 0.5f * (waveManager->GetCurrentWave());
		pDustEmitter->Desc.SizeMin = 0.3f * (waveManager->GetCurrentWave());
...
void SceneMain::SpawnEnemies(float deltaTime)
{
	...
	int maxLimit = isBossWave ? ((int)enemyPool->GetBossGroupData().get()->meleeFear+1)*(int)finalChargeEnemyNum: waveManager->GetMaxSpawnLimit();
...
void SceneMain::UpdateWaveTransition()
{
	...
	if (m_gameState==GameState::Play&&(waveManager->GetState() == WaveManager::WaveState::ClearWait) && waveManager->GetCurrentWave() < waveManager->GetMaxWave())
	{
		...
		if (vnKeyboard::on(DIK_RETURN) || vnMouse::onR())
		{
			...
			{
				//ボスのチュートリアルを出す
				ChangeTutorialState(ExplanationType::Boss,true);
...
void SceneMain::UpdateLevelUp()
{
	...
	if (m_pUpgradeUI->GetIsClosingUI() && m_pUpgradeUI->GetIsFinishAnim())
	{
		...
		{
			enemyPool->ShowHideAllActiveEnemies(true);
			m_pNewPlayer->FinishLevelUp();
			m_pUpgradeUI->ResetPhase();
			m_pBlockManager->AllBlocksShowHide(true,waveManager->GetFinalWave());
